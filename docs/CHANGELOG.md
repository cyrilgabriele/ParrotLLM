...
### [2026-05-05] - Tilman Haferbeck

#### What
- Added the benchmark-driven posttraining path as the best-performing direction for the current evaluation target: SFT and DPO stages, typed configs, CLI stages, masked response-only SFT loss, DPO with a frozen reference model, benchmark decontamination, catastrophic-forgetting tripwires, and leaderboard inference fixes.
- Preserved the LAMBADA-oriented inference behavior by keeping leaderboard prompt `rstrip()` and cloze scoring intact.
- Added the v9 hybrid MC-balanced SFT recipe (`configs/post_training/sft_v9_mc_balanced.yaml`) and builder (`tools/build_hybrid_mc_sft_mix.py`): public train-split MC data provides the main signal, while a small control layer enforces raw prompt format and answer-letter balance.
- Added tests for the hybrid MC builder contract so answer-letter placement stays deterministic and balanced.

#### Why
- The benchmark profile shows very strong LAMBADA performance and competitive HellaSwag/WinoGrande, while OpenBookQA is effectively random; the best next move is to keep the LAMBADA-safe inference path fixed and improve MC behavior with train-split task-shaped data.
- A hybrid MC mix is safer than broad synthetic training because it uses real train-split examples for task distribution and only uses controlled examples for mechanical calibration.

#### Remarks
- Final checkpoint selection should be gated by public benchmark average with hard guards: LAMBADA within 1.0 point of the current best, HellaSwag/WinoGrande regression at most 0.5, and OpenBookQA above 27.0.
