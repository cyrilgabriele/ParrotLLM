...
### 12.4 v9: hybrid MC-balanced mix

- **Source:** `configs/post_training/sft_v9_mc_balanced.yaml`; `tools/build_hybrid_mc_sft_mix.py`; benchmark chart showing LAMBADA as the strongest public result and OpenBookQA at random-level accuracy.
- **Reason:** The best next SFT iteration should not flood training with artificial MC rows. The main signal comes from public train splits in the same task families as the benchmark, while a small control layer only fixes mechanical issues: raw prompt format, option shuffling, and answer-letter balance. This keeps the LAMBADA-safe inference path fixed and targets the observed MC weakness directly.
